(function($){
  ...
  jQuery(document).on('ready',function(){
    function initNav(){
      ...
      function initMap() {
        var uluru = {lat: 11.1712086, lng: 76.3409995};
...
function initMail(){
      $('form#contact-form').on('submit',function(e){
        ...
        $.ajax({
          type:'POST',url:'php/mail_handler.php',data:post_data
        }).done(function(){
  ...
  jQuery(window).on('load',function(){
    ...
    sr.reveal('#v-card-holder',{
      duration:1400,distance:'150px'
    });
    sr.reveal('.skillbar-bar',{
      duration:1800,delay:300,distance:'0'
    });
